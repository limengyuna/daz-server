...
public class ChatController {
    ...
    public Result<ChatConversationVO> getOrCreateConversation(@RequestParam("targetUserId") Long targetUserId) {
        Long userId = UserContextHolder.getPrincipalId();
        if (userId == null) {
            return Result.error("未登录或 Token 无效");
        }
    ...
    public Result<ChatMessageVO> sendPrivateMessage(@Valid @RequestBody SendMessageRequest request) {
        Long userId = UserContextHolder.getPrincipalId();
        if (userId == null) {
            return Result.error("未登录或 Token 无效");
        }
    ...
    public Result<ChatMessageVO> sendGroupMessage(@Valid @RequestBody SendMessageRequest request) {
        Long userId = UserContextHolder.getPrincipalId();
        if (userId == null) {
            return Result.error("未登录或 Token 无效");
        }
